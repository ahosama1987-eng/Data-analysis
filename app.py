...
PALETTE = [PBI_ACCENT, PBI_BLUE, PBI_GREEN, PBI_ACCENT2, PBI_PURPLE,
           PBI_TEAL, PBI_ORANGE, "#ef9a9a", "#80cbc4", "#ffcc02",
           "#b39ddb", "#80deea", "#a5d6a7", "#ffe082", "#f48fb1"]
...
<div class="app-footer">
    DATAVISION PRO &nbsp;·&nbsp; EXPERT EDITION v3.0 &nbsp;·&nbsp; LARGE-FILE READY
    &nbsp;·&nbsp; STREAMLIT · PLOTLY · CLAUDE AI &nbsp;·&nbsp; {datetime.now().year}
...
def pbi_section(label):
    ...
    <div class="pbi-section">
        <div class="pbi-section-label">{label}</div>
        <div class="pbi-section-line"></div>
    </div>""", unsafe_allow_html=True)

def pbi_card(title, dot_color=None):
    if dot_color is None: dot_color = PBI_ACCENT
    st.markdown(f"""
    <div class="pbi-card">
        <div class="pbi-card-header">
            <div class="pbi-card-title">
                <div class="pbi-card-dot" style="--dot:{dot_color}"></div>
                {title}
            </div>
            <span class="pbi-card-menu">⋯</span>
        </div>
...
with st.sidebar:
    st.markdown(f"""
    <div style="padding:14px 2px 10px;border-bottom:1px solid {PBI_BORDER};margin-bottom:10px">
        <div style="font-size:0.6rem;font-weight:700;color:{PBI_ACCENT};
                    text-transform:uppercase;letter-spacing:2px;margin-bottom:2px">&#9646; Filter Panel</div>
        <div style="font-size:0.68rem;color:{PBI_MUTED}">Slice your data</div>
    </div>""", unsafe_allow_html=True)
...
<div class="pbi-topbar">
    <div class="pbi-logo">
        <div class="pbi-logo-icon">DV</div>
        <div>
            <div class="pbi-logo-text">DataVision Pro</div>
            <div class="pbi-logo-sub">Enterprise Analytics</div>
        </div>
    </div>
    <div class="pbi-topbar-right">
        <span class="pbi-badge">EXPERT</span>
        <span class="pbi-time-badge">&#x1F550; {_now_str}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
if uploaded is None and not use_sample:
    ...
    <div class="pbi-welcome-grid">
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F4C2;</div>
            <div class="pbi-welcome-title">Upload Any File</div>
            <div class="pbi-welcome-desc">CSV or Excel — 100 MB+ with chunked loading</div></div>
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F4CA;</div>
            <div class="pbi-welcome-title">15+ Chart Types</div>
            <div class="pbi-welcome-desc">Bar, Line, Scatter, Treemap, Donut, Pareto &amp; more</div></div>
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F52E;</div>
            <div class="pbi-welcome-title">AI Forecasting</div>
            <div class="pbi-welcome-desc">Time-series forecasting with confidence bands</div></div>
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F916;</div>
            <div class="pbi-welcome-title">AI Chat Analyst</div>
            <div class="pbi-welcome-desc">Ask in English or Arabic</div></div>
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F9F9;</div>
            <div class="pbi-welcome-title">Data Cleaning</div>
            <div class="pbi-welcome-desc">Duplicates, missing values, renames</div></div>
        <div class="pbi-welcome-card"><div class="pbi-welcome-icon">&#x1F4E4;</div>
            <div class="pbi-welcome-title">Export Center</div>
            <div class="pbi-welcome-desc">CSV, Excel, and interactive charts</div></div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
with tab_overview:
    ...
    st.markdown(f"""
    <div class="pbi-qs-wrap">
        <div class="pbi-qs-ring" style="--qc:{qc};--qc-glow:{qcg}">{qs:.0f}</div>
        <div class="pbi-qs-detail">
            <h3>{"✅" if qs>=80 else "⚠️"} Data Quality: {ql}</h3>
            <p>{len(df_raw):,} rows · {len(df_raw.columns)} columns analyzed</p>
            <div class="pbi-pills">
                <span class="pbi-pill">📋 {len(df_raw):,} rows</span>
                <span class="pbi-pill">📑 {len(df_raw.columns)} cols</span>
                <span class="pbi-pill">❓ {df_raw.isnull().mean().mean()*100:.1f}% missing</span>
                <span class="pbi-pill">🔁 {df_raw.duplicated().sum()} dups</span>
                <span class="pbi-pill">📊 {len(num_cols)} numeric</span>
                <span class="pbi-pill">🏷️ {len(cat_cols)} text</span>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if num_cols:
        ...
        cards_html = '<div class="pbi-kpi-row">'
        for i, col in enumerate(num_cols[:8]):
            ...
            cards_html += f"""
            <div class="pbi-kpi" style="--kc:{kc}">
                <div class="pbi-kpi-icon">{icons[i%len(icons)]}</div>
                <div class="pbi-kpi-label">{col}</div>
                <div class="pbi-kpi-value">{fmt(total)}</div>
                <div class="pbi-kpi-delta neutral">Avg {fmt(mean_)} &nbsp;·&nbsp; Max {fmt(mx_v)}</div>
            </div>"""
        cards_html += "</div>"
    ...
    ins_html = '<div class="pbi-insights-grid">'
    for icon, color, text in insights:
        clean = text.replace("**","<b>",1)
        while "**" in clean:
            clean = clean.replace("**","</b>",1).replace("**","<b>",1)
        ins_html += f'<div class="pbi-insight" style="--ic:{color}"><div class="pbi-insight-icon">{icon}</div><div>{clean}</div></div>'
    ins_html += "</div>"
