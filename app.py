...
def safe_read(f, file_name=""):
    """Read CSV or Excel from a file-like object. file_name used for format detection."""
    ...
    if name.endswith(".csv"):
        ...
        f.seek(0)
        try:
            import python_calamine  # noqa — fast Rust engine
            engine = "calamine"
        except ImportError:
            engine = "openpyxl"
...
#  LOAD DATA
# ══════════════════════════════════════════════════════════════
@st.cache_data(show_spinner="⚡ Loading data…", ttl=3600, max_entries=3)
def load_data(file_bytes: bytes, file_name: str):
    import io as _io
    buf = _io.BytesIO(file_bytes)
    sheets = safe_read(buf, file_name)
    for k in list(sheets.keys()):
        df_s = sheets[k]
        df_s = detect_and_parse_dates(df_s)
        for col in df_s.select_dtypes(include=["int64","int32"]).columns:
            df_s[col] = pd.to_numeric(df_s[col], downcast="integer")
        for col in df_s.select_dtypes(include=["float64"]).columns:
            df_s[col] = pd.to_numeric(df_s[col], downcast="float")
        sheets[k] = df_s
    return sheets
...
if use_sample:
    ...
    try:
        ...
        if file_mb > 50:
            st.info(f"📦 Large file ({file_mb:.1f} MB) — loading…")
        with st.spinner(f"⚡ Reading {uploaded.name} ({file_mb:.1f} MB)…"):
            sheets = load_data(file_bytes, uploaded.name)
        total_rows = sum(len(v) for v in sheets.values())
        st.success(f"✅ **{uploaded.name}** — {total_rows:,} rows · {len(sheets)} sheet(s) · {file_mb:.1f} MB")
    except Exception as e:
        st.error(f"❌ **File read error:** {e}")
        st.info("Tips: save CSV as UTF-8, ensure Excel has at least one data sheet.")
        st.stop()
