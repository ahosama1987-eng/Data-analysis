...
# ══════════════════════════════════════════════════════════════
#  PAGE CONFIG  — raise Streamlit upload limit to 500 MB
# ══════════════════════════════════════════════════════════════
st.set_page_config(
...
# ── raise the per-upload cap via env var so Streamlit accepts it ──
# (set STREAMLIT_SERVER_MAX_UPLOAD_SIZE=500 in your environment,
#  or run: streamlit run app.py --server.maxUploadSize=500)
...
.app-header-logo {{ font-size: 2.4rem; filter: drop-shadow(0 0 12px rgba(242,201,76,0.4)); }}
.app-header h1 {{
    color: {GOLD}; font-size: 1.6rem; font-weight: 800;
    margin: 0 0 3px; letter-spacing: -0.02em;
}}
.app-header p {{ color: {MUTED}; margin: 0; font-size: 0.8rem; }}
.app-header-meta {{ margin-left: auto; text-align: right; }}
.app-header-meta .badge {{
    background: linear-gradient(135deg, {GOLD}, {ORANGE});
    color: #0f0f1a; font-size: 0.6rem; font-weight: 800;
    padding: 3px 10px; border-radius: 20px;
    letter-spacing: 1.5px; text-transform: uppercase;
}}
.app-header-meta .date-str {{ color: {MUTED}; font-size: 0.75rem; margin-top: 5px; }}

.sidebar-logo {{
    padding: 20px 16px 16px; border-bottom: 1px solid {BORDER}; margin-bottom: 4px;
}}
.sidebar-logo h2 {{ color: {GOLD}; font-size: 1.1rem; font-weight: 700; margin: 0; }}
.sidebar-logo span {{ color: {MUTED}; font-size: 0.72rem; }}

.plan-card {{
    background: linear-gradient(135deg, {CARD}, {CARD2});
    border: 1px solid {BORDER}; border-radius: 10px; padding: 12px 14px; margin: 8px 0;
}}
...
.sec-header {{
    ...
    margin: 24px 0 14px; padding-bottom: 8px; border-bottom: 1px solid {BORDER};
}}
.sec-header .icon {{ font-size: 1.1rem; }}
.sec-header .title {{
    color: {GOLD}; font-size: 0.82rem; font-weight: 700;
    text-transform: uppercase; letter-spacing: 1.5px;
}}
.sec-header .count {{
    background: {CARD2}; color: {MUTED}; font-size: 0.68rem;
    padding: 2px 8px; border-radius: 10px; border: 1px solid {BORDER}; margin-left: auto;
}}
...
.kpi-card::before {{
    ...
    background: var(--accent, {GOLD}); border-radius: 14px 14px 0 0;
}}
.kpi-card::after {{
...
.kpi-label {{
    ...
    letter-spacing: 1.5px; font-weight: 500; margin-bottom: 4px;
}}
.kpi-value {{ color: #fff; font-size: 1.8rem; font-weight: 800; line-height: 1; letter-spacing: -0.03em; }}
.kpi-sub {{ color: {MUTED}; font-size: 0.7rem; margin-top: 6px; display: flex; gap: 6px; }}
.kpi-sub .up   {{ color: {GREEN}; }}
.kpi-sub .down {{ color: {RED}; }}
.kpi-trend {{ position: absolute; bottom: 14px; right: 16px; font-size: 0.7rem; font-weight: 600; }}

.insight-card {{
    background: {CARD}; border: 1px solid {BORDER};
    border-left: 3px solid var(--c, {GOLD}); border-radius: 10px;
    padding: 14px 16px; margin-bottom: 10px; font-size: 0.84rem; color: #cbd5e1;
...
.stTabs [data-baseweb="tab-list"] {{
    background: transparent; border-bottom: 2px solid {BORDER}; gap: 2px; padding: 0 0 0 2px;
}}
.stTabs [data-baseweb="tab"] {{
    color: {MUTED}; font-weight: 500; font-size: 0.82rem;
    padding: 10px 18px; border-radius: 8px 8px 0 0; border: none; transition: all 0.15s;
}}
.stTabs [data-baseweb="tab"]:hover {{ color: {TEXT}; background: {CARD}; }}
.stTabs [aria-selected="true"] {{
    color: {GOLD} !important; background: {CARD} !important;
    border-bottom: 2px solid {GOLD}; font-weight: 700 !important;
}}

[data-testid="stMetric"] {{
    background: {CARD}; padding: 16px; border-radius: 12px;
    border: 1px solid {BORDER}; border-top: 2px solid {GOLD}; transition: transform 0.15s;
}}
...
.stTextInput label, .stNumberInput label {{
    color: {TEXT} !important; font-size: 0.8rem !important; font-weight: 500 !important;
}}
div[data-baseweb="select"] > div {{
    background: #1a1a2e !important; border: 1px solid {BORDER} !important;
    border-radius: 8px !important; color: {TEXT} !important;
}}
div[data-baseweb="select"] > div:focus-within {{
    border-color: {GOLD} !important; box-shadow: 0 0 0 2px rgba(242,201,76,0.15) !important;
}}
.stTextInput input, .stNumberInput input {{
    background: #1a1a2e !important; border: 1px solid {BORDER} !important;
    border-radius: 8px !important; color: {TEXT} !important; padding: 9px 12px !important;
}}
...
.stButton > button {{
    background: linear-gradient(135deg, {GOLD}, {ORANGE}); color: #0f0f1a;
    font-weight: 700; border: none; border-radius: 8px; padding: 9px 24px;
    font-size: 0.82rem; letter-spacing: 0.02em; transition: all 0.2s;
    box-shadow: 0 4px 12px rgba(242,201,76,0.2);
}}
.stButton > button:hover {{
    opacity: 0.9; transform: translateY(-2px); box-shadow: 0 6px 18px rgba(242,201,76,0.35);
}}
.stButton > button:active {{ transform: translateY(0); }}
.stDownloadButton > button {{
    background: transparent; color: {GOLD}; border: 1px solid {GOLD};
    border-radius: 8px; padding: 8px 20px; font-size: 0.8rem; font-weight: 600; transition: all 0.2s;
}}
.stDownloadButton > button:hover {{
    background: rgba(242,201,76,0.1); box-shadow: 0 0 12px rgba(242,201,76,0.2);
}}

[data-testid="stFileUploader"] {{
    background: #141428; border: 2px dashed {BORDER}; border-radius: 12px;
...
.quality-banner {{
    background: linear-gradient(135deg, {CARD}, {CARD2});
    border: 1px solid {BORDER}; border-radius: 14px; padding: 20px 24px;
...
.quality-score-ring {{
    ...
    font-size: 1.15rem; font-weight: 800; color: var(--qc, {GREEN}); flex-shrink: 0;
}}
.quality-details h3 {{ color: #fff; font-size: 1rem; margin: 0 0 4px; }}
.quality-details p  {{ color: {MUTED}; font-size: 0.78rem; margin: 0 0 8px; }}
.quality-pills {{ display: flex; flex-wrap: wrap; gap: 6px; }}
.quality-pill {{
    background: {CARD2}; border: 1px solid {BORDER}; border-radius: 20px;
    padding: 3px 10px; font-size: 0.7rem; color: {TEXT};
}}
...
.welcome-card:hover {{ transform: translateY(-4px); border-color: {GOLD}; }}
.welcome-icon {{ font-size: 2rem; margin-bottom: 10px; }}
.welcome-title {{ color: {GOLD}; font-weight: 700; font-size: 0.9rem; margin-bottom: 6px; }}
.welcome-desc  {{ color: {MUTED}; font-size: 0.78rem; line-height: 1.4; }}
...
.bubble.user {{
    background: linear-gradient(135deg, {GOLD}, {ORANGE});
    color: #0f0f1a; font-weight: 500; border-radius: 16px 16px 4px 16px;
}}
.bubble.ai {{
...
.chat-avatar.user {{ background: linear-gradient(135deg, {GOLD}, {ORANGE}); }}
.chat-avatar.ai   {{ background: {CARD2}; border: 1px solid {BORDER}; }}
.chat-empty {{
    text-align: center; padding: 40px; color: {MUTED};
    background: {CARD}; border-radius: 14px; border: 1px solid {BORDER};
}}
.chat-empty .icon {{ font-size: 2.5rem; margin-bottom: 10px; }}
.chat-empty h3    {{ color: {GOLD}; margin: 0 0 8px; font-size: 1rem; }}
.chat-empty p     {{ font-size: 0.82rem; line-height: 1.5; margin: 0; }}

.app-footer {{
    text-align: center; padding: 16px;
    color: #2d2d50; font-size: 0.72rem;
    border-top: 1px solid {BORDER}; margin-top: 24px;
}}
.streamlit-expanderHeader {{
    background: {CARD} !important; border-radius: 8px !important;
    color: {TEXT} !important; font-size: 0.82rem !important;
}}
...
#  CONSTANTS
# ══════════════════════════════════════════════════════════════
# Rows to keep in memory for charts / analysis when dataset is huge.
# The full file is still counted for KPIs via chunked aggregation.
DISPLAY_ROW_LIMIT = 500_000   # rows kept in RAM for interactive use
CHUNK_SIZE        = 100_000   # rows per chunk when reading large CSVs
...
def detect_and_parse_dates(df):
    """Fast date detection — sample 200 rows only."""
...
def downcast_df(df):
    """Reduce memory by downcasting numeric dtypes in-place."""
...
def auto_insights(df, num_cols, cat_cols, date_cols):
    ...
    for col in cat_cols[:2]:
        try:
            col_data = df[col].astype(str) if hasattr(df[col], 'cat') else df[col]
...
def generate_sample_data():
    ...
    df = pd.DataFrame({
        ...
        "Revenue":    None, "Cost": None, "Profit": None,
...
def forecast_chart(hist_df, fc_df, date_col, val_col):
    ...
    fig.add_trace(go.Scatter(x=hist_df[date_col], y=hist_df[val_col],
                             name="Historical", line=dict(color=BLUE, width=2.5), mode="lines"))
...
# ══════════════════════════════════════════════════════════════
#  ★ NEW: FAST CHUNKED FILE READER  (supports 100 MB+)
# ══════════════════════════════════════════════════════════════
def _read_csv_chunked(buf, encoding="utf-8"):
    """
    Read a CSV in CHUNK_SIZE-row chunks, then concatenate.
    Uses dtype inference only on the first chunk; subsequent
    chunks are cast to match → much faster on huge files.
    """
    chunks = []
    dtypes = None
    reader = pd.read_csv(
    ...
    for chunk in reader:
        if dtypes is None:
            ...
            # Cast to already-detected dtypes to avoid re-inference overhead
...
@st.cache_data(show_spinner="📊 Loading data…", ttl=3600,
               max_entries=3)          # keep at most 3 files cached
def load_data(file_bytes: bytes, file_name: str):
    """
    Cache the raw file bytes → parsed DataFrames dict.
    Works for CSV and Excel; handles files > 100 MB via chunking.
    """
    import io as _io
    ...
    if name.endswith(".csv"):
        for enc in ["utf-8", "utf-8-sig", "latin-1", "cp1252"]:
            try:
                buf.seek(0)
                df = _read_csv_chunked(buf, encoding=enc)
                return {"Sheet1": df}
        ...
        # Excel: use calamine engine when available (much faster), else openpyxl
        try:
            import python_calamine  # noqa – fast Rust-based reader
            engine = "calamine"
        except ImportError:
            engine = "openpyxl"
...
with st.sidebar:
    ...
    st.markdown("---")
    # ── Raise Streamlit upload cap hint ──
    st.markdown(
        f'<p style="color:{MUTED};font-size:0.72rem;margin-bottom:4px">'
        '💡 For files >200 MB run:<br>'
        '<code>streamlit run app.py --server.maxUploadSize=500</code></p>',
        unsafe_allow_html=True,
    )
    uploaded = st.file_uploader(
        "📁 Upload Dataset",
        type=["csv", "xlsx"],
        help="CSV or Excel — multi-sheet supported. Large files (100 MB+) use chunked loading.",
    )
    ...
    st.markdown("---")
    st.caption("DataVision Pro v3.0 · Large-File Edition")
...
<div class="app-header">
    ...
    <div>
        ...
        <p>Power BI-Style Analytics &nbsp;·&nbsp; Upload CSV / Excel to begin &nbsp;·&nbsp; Supports 100 MB+ files</p>
    </div>
...
if uploaded is None and not use_sample:
    features = [
        ("📂", "Upload Any File", "CSV or Excel — now supports 100 MB+ files"),
        ("⚡", "Chunked Loading", "Large files parsed in fast 100 K-row chunks"),
    ...
    try:
        ...
        if file_mb > 50:
            st.info(
                ...
                "using chunked loading for optimal performance. Please wait…"
            )

        with st.spinner(f"📊 Loading {uploaded.name} ({file_mb:.1f} MB)…"):
...
# ── Row-limit enforcement (plan-based) ──
total_raw_rows = len(df_raw)
if total_raw_rows > ROW_LIMIT:
    st.warning(
        f"⚠️ **{PLAN} plan limit:** Your file has **{total_raw_rows:,} rows** "
        f"but plan allows **{ROW_LIMIT:,}**. Showing first {ROW_LIMIT:,} rows."
    )
    df_raw = df_raw.head(ROW_LIMIT)

# ── For very large datasets, keep a display-safe subset for charts ──
LARGE_FILE = total_raw_rows > DISPLAY_ROW_LIMIT
if LARGE_FILE and PLAN == "Business":
    st.info(
        f"📊 **Large dataset ({total_raw_rows:,} rows):** "
        f"Charts use a representative **{DISPLAY_ROW_LIMIT:,}-row** sample for speed. "
...
# Convert any remaining category columns to string for filter widgets
for col in cat_cols:
...
# ── Chart-safe slice (avoids sluggish plots on millions of rows) ──
@st.cache_data(show_spinner=False)
def _chart_sample(df_bytes_hash, n=DISPLAY_ROW_LIMIT):
    """Return a deterministic sample for charting large datasets."""
    return df.sample(min(n, len(df)), random_state=42) if len(df) > n else df

df_chart = _chart_sample(len(df)) if len(df) > DISPLAY_ROW_LIMIT else df
...
with tab_overview:
    ...
    if cat_cols and num_cols:
        ...
        with c2:
            try:
                d   = safe_groupby(df_chart, [cat_cols[0]], num_cols[0])
                fig = donut(d, cat_cols[0], num_cols[0], title=f"{num_cols[0]} Share")
                st.plotly_chart(fig, use_container_width=True)
            except Exception as e:
                st.warning(f"Chart error: {e}")
    ...
    try:
        if chart_type in ["Horizontal Bar","Vertical Bar","Grouped Bar",
            ...
            if chart_type in ["Horizontal Bar","Vertical Bar","Grouped Bar"]:
                ...
                fig = funnel(df_chart, x_col, y_col, f"{y_col} Funnel")
            ...
            sample = df_chart.sample(min(2000, len(df_chart)), random_state=42)
...
with tab_analysis:
    ...
    if num_cols:
        try:
            ...
            stats["missing%"] = (df_chart[num_cols].isnull().mean() * 100).round(1)
            stats["outliers"] = [detect_outliers_iqr(df_chart[c].dropna()).sum() for c in num_cols]
    ...
    if PLAN == "Free":
        ...
        if st.button("🔮 Run Forecast", key="run_fc"):
            with st.spinner("Running forecast model…"):
                try:
                    result = forecast_series(df_chart, fc_date, fc_val, periods=fc_per)
                    if result[0] is None:
                        st.warning(result[1])
                    else:
...
with tab_deep:
    ...
    if num_cols:
        ...
        try:
            ...
            with c2:
                ...
                if n_out:
                    st.dataframe(df_chart[mask].head(20), use_container_width=True)
        except Exception as e:
            st.warning(f"Outlier error: {e}")

    sec("❓", "Missing Value Analysis")
    miss_summary = df_chart.isnull().sum()
    miss_pct     = (df_chart.isnull().mean() * 100).round(1)
    miss_df      = pd.DataFrame({"Column": df_chart.columns, "Missing": miss_summary, "Pct": miss_pct})
    ...
    if len(miss_df):
        ...
        with c2:
            try:
                fig_m = missing_heatmap(df_chart.sample(min(200, len(df_chart)), random_state=1))
                if fig_m:
        ...
        if dist_cols:
            try:
                ...
                for i, col in enumerate(dist_cols):
                    vals = df_chart[col].dropna()
                    c_color = PALETTE[i % len(PALETTE)]
                    fig_dist.add_trace(
                        go.Violin(y=vals, name=col, box_visible=True,
                                  meanline_visible=True, fillcolor=hex_to_rgba(c_color, 0.3),
                                  line_color=c_color),
                        row=1, col=i+1
                    )
                layout = {k: v for k, v in PBI_LAYOUT.items() if k not in ("xaxis","yaxis")}
                fig_dist.update_layout(height=380, showlegend=False, **layout)
...
with tab_clean:
    ...
    with c1:
        if st.button("🗑️ Remove Duplicates", use_container_width=True):
            ...
            st.success(f"✅ Removed {before - len(dfc):,} duplicate rows.")
    with c2:
        fill_strategy = st.selectbox("Fill missing with", ["Mean", "Median", "Zero", "Drop rows"], key="fill_s")
        if st.button("🔧 Fill Missing Values", use_container_width=True):
            n_before = dfc.isnull().sum().sum()
            nc2 = get_num_cols(dfc)
            if fill_strategy == "Mean":
                dfc[nc2] = dfc[nc2].fillna(dfc[nc2].mean())
            elif fill_strategy == "Median":
                dfc[nc2] = dfc[nc2].fillna(dfc[nc2].median())
            elif fill_strategy == "Zero":
                dfc[nc2] = dfc[nc2].fillna(0)
            elif fill_strategy == "Drop rows":
                dfc = dfc.dropna()
            st.session_state.df_clean = dfc
            st.success(f"✅ Fixed {n_before - dfc.isnull().sum().sum():,} missing values.")
    with c3:
        if st.button("🔄 Reset to Original", use_container_width=True):
            st.session_state.df_clean = df_raw.copy()
            dfc = st.session_state.df_clean
            st.success("↩️ Reset to original data.")

    st.markdown("##### ✏️ Rename Columns")
    rename_cols = st.multiselect("Select columns to rename", dfc.columns.tolist(), key="rename_sel")
    if rename_cols:
        rename_map = {}
        ...
        if rename_map and st.button("✅ Apply Renames"):
            dfc = dfc.rename(columns=rename_map)
            st.session_state.df_clean = dfc
            st.success(f"✅ Renamed {len(rename_map)} column(s).")

    st.markdown("##### 🗑️ Drop Columns")
    drop_sel = st.multiselect("Columns to drop", dfc.columns.tolist(), key="drop_sel")
    if drop_sel and st.button("Drop Selected Columns", key="drop_btn"):
        dfc = dfc.drop(columns=drop_sel)
        st.session_state.df_clean = dfc
        st.success(f"✅ Dropped {len(drop_sel)} column(s).")

    st.markdown("##### 📋 Cleaned Data Preview")
    st.dataframe(dfc.head(100), use_container_width=True, height=250)
    st.caption(f"{len(dfc):,} rows · {len(dfc.columns)} columns")
    ...
    with dl2:
        try:
            st.download_button("⬇️ Download Cleaned Excel",
                               to_excel(dfc),
                               "cleaned_data.xlsx",
...
# ══════════════════════════════════════════════════════════════
with tab_data:
    sec("🗃️", "Data Explorer", f"{len(df):,} rows")

    search_term = st.text_input("🔍 Search rows (any column)", placeholder="Type to filter…", key="dt_search")
    df_view = df
    if search_term:
        mask_search = df.astype(str).apply(lambda col: col.str.contains(search_term, case=False, na=False)).any(axis=1)
        df_view = df[mask_search]
        st.caption(f"🔎 {len(df_view):,} rows matched '{search_term}'")

    n_show = st.slider("Rows to display", 50, min(2000, len(df_view)), 200, step=50, key="dt_nshow")
    st.dataframe(df_view.head(n_show), use_container_width=True, height=450)
...
with tab_export:
    ...
    with e1:
        st.download_button("📄 Full CSV",
                           ...
                           "datavision_export.csv", "text/csv", use_container_width=True)
    with e2:
        try:
            st.download_button("📊 Full Excel",
                               to_excel(df),
                               "datavision_export.xlsx",
                               "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                               use_container_width=True)
        except Exception as e:
            st.warning(f"Excel export error: {e}")
    with e3:
        try:
            html_report = build_html_report(df_chart, num_cols, cat_cols, date_cols, fname, sheet_name)
            st.download_button("📑 HTML Report",
                               html_report, "datavision_report.html",
    ...
    if num_cols:
        try:
            stats_exp = df_chart[num_cols].describe().T.round(2)
            stats_exp["skewness"] = df_chart[num_cols].skew().round(2)
            stats_exp["kurtosis"] = df_chart[num_cols].kurt().round(2)
            st.dataframe(stats_exp, use_container_width=True)
        except Exception as e:
            st.warning(f"Stats error: {e}")
...
with tab_chat:
    ...
    data_ctx   = build_data_context(df_chart, num_cols, cat_cols, date_cols)
    suggestions = []
    if num_cols:
...
<div class="app-footer">
    DataVision Pro · Expert Edition v3.0 · Large-File Ready · Powered by Streamlit · Plotly · Claude AI &nbsp;·&nbsp; {datetime.now().year}
</div>""", unsafe_allow_html=True)
