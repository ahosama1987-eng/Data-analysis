...
PBI_BG      = "#0e1117"   # near-black canvas
PBI_SURFACE = "#1c2333"   # card surface
PBI_TOPBAR  = "#111827"   # top nav
PBI_SIDEBAR = "#161d2e"   # sidebar
PBI_BORDER  = "#2d3748"   # borders
...
PBI_RED     = "#f87171"   # red
PBI_TEXT    = "#f1f5f9"   # off-white text
...
PBI_LAYOUT = dict(
    ...
    font=dict(color=PBI_TEXT, family="'DM Sans','Segoe UI',sans-serif", size=11),
    title_font=dict(color=PBI_TEXT, size=13, family="'DM Sans','Segoe UI',sans-serif"),
...
_CHUNK = 150_000   # rows per chunk

def _downcast(df):
    """Shrink numeric dtypes in-place — cuts RAM 30-60%."""
...
def _csv_chunked(buf, encoding="utf-8"):
...
def safe_read(f, file_name=""):
    """Read CSV or Excel from a file-like object. Chunked for large files."""
    name = (file_name or getattr(f, "name", "") or "").lower()
    if name.endswith(".csv"):
        for enc in ["utf-8", "utf-8-sig", "latin-1", "cp1252"]:
            try:
                f.seek(0)
                df = _csv_chunked(f, encoding=enc)
                return {"Sheet1": df}
        ...
        f.seek(0)
        try:
            import python_calamine  # noqa — fast Rust-based engine (~5x faster than openpyxl)
            engine = "calamine"
        except ImportError:
            engine = "openpyxl"
...
<div class="dv-topbar">
    ...
    <div class="dv-topbar-right">
        <span class="dv-pill dv-pill-accent">Expert</span>
...
<div style="background:rgba(99,102,241,0.08);border:1px solid {PBI_BORDER};
    ...
    <div>RAM usage: <b style="color:{PBI_TEXT}">{mem_mb:.1f} MB</b></div>
        ...
        pass
    st.caption("DataVision Pro v3.0 · Large-File Ready")
...
if uploaded is None and not use_sample:
    ...
    <div class="dv-welcome-grid">
        <div class="dv-welcome-card" style="--wc1:{PBI_ACCENT};--wc2:{PBI_PURPLE}">
            ...
            <div class="dv-welcome-desc">CSV or Excel — multi-sheet supported, 100 MB+ ready</div>
        </div>
...
def load_data(file_bytes: bytes, file_name: str):
    """
    Cache keyed on (file_bytes, file_name).
    _downcast is already called inside safe_read per chunk so no
    need to iterate columns again here.
    """
    import io as _io
    buf = _io.BytesIO(file_bytes)
    return safe_read(buf, file_name)
    ...
    try:
        ...
        if file_mb > 500:
            st.warning(
                ...
                "For files over 200 MB, run: `streamlit run app.py --server.maxUploadSize=1024`"
            )
        elif file_mb > 100:
            st.info(f"📦 **Large file ({file_mb:.1f} MB)** — using chunked streaming. May take 10-30s…")
        ...
        st.success(
            ...
            f"**{total_rows:,} rows** · {len(sheets)} sheet(s) · {file_mb:.1f} MB"
        )
    except Exception as e:
...
# ── Chart-safe sample ────────────────────────────────────────────────────
# Charts work on df_chart (≤200K rows) for speed.
# KPI totals still use full df.
_CHART_CAP = 200_000
if len(df) > _CHART_CAP:
    st.info(
        f"⚡ **Large dataset ({len(df):,} rows):** charts use a "
        f"**{_CHART_CAP:,}-row** representative sample for speed. "
        "KPIs reflect the full dataset."
    )
    df_chart = df.sample(_CHART_CAP, random_state=42)
else:
    df_chart = df

# ── Cached groupby helper ────────────────────────────────────────────────
@st.cache_data(show_spinner=False, ttl=300, max_entries=50)
def _cgb(df_key: int, group_cols: tuple, val_col: str, agg: str = "sum"):
    """Cached groupby — df_key is len(df_chart) used as a cheap hash proxy."""
...
def cgb(group_cols, val_col, agg="sum"):
    ...
    return _cgb(len(df_chart), cols, val_col, agg)
