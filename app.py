...
PBI_BG      = "#090d18"   # deep navy canvas
PBI_SURFACE = "#111827"   # card surface
PBI_TOPBAR  = "#0d1220"   # top nav bar
PBI_SIDEBAR = "#0f1521"   # sidebar
PBI_BORDER  = "#1e2d45"   # refined borders
PBI_ACCENT  = "#6366f1"   # indigo primary
PBI_ACCENT2 = "#f59e0b"   # amber
PBI_BLUE    = "#38bdf8"   # sky blue
PBI_GREEN   = "#34d399"   # emerald
PBI_PURPLE  = "#a78bfa"   # violet
PBI_TEAL    = "#2dd4bf"   # teal
PBI_ORANGE  = "#fb923c"   # orange
PBI_PINK    = "#f472b6"   # pink
PBI_RED     = "#f87171"   # red
PBI_TEXT    = "#e2e8f0"   # slightly softer white
PBI_MUTED   = "#64748b"   # slate muted
PBI_WHITE   = "#ffffff"
PBI_LIME    = "#a3e635"   # lime accent
...
PALETTE = [PBI_ACCENT, PBI_BLUE, PBI_GREEN, PBI_ACCENT2, PBI_PURPLE,
           PBI_TEAL, PBI_ORANGE, PBI_PINK, PBI_LIME, PBI_RED,
           "#67e8f9", "#fde68a", "#bbf7d0", "#ddd6fe", "#fed7aa"]

PBI_LAYOUT = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(0,0,0,0)",
    font=dict(color=PBI_TEXT, family="'Inter','Segoe UI',sans-serif", size=11),
    title_font=dict(color=PBI_TEXT, size=13, family="'Inter','Segoe UI',sans-serif"),
    legend=dict(bgcolor="rgba(22,33,62,0.95)", bordercolor=PBI_BORDER,
                borderwidth=1, font=dict(color=PBI_MUTED, size=10),
                orientation="h", yanchor="bottom", y=-0.28, xanchor="center", x=0.5),
    margin=dict(l=10, r=10, t=36, b=10),
    colorway=PALETTE,
    xaxis=dict(gridcolor="rgba(42,42,90,0.6)", linecolor=PBI_BORDER,
               tickfont=dict(color=PBI_MUTED, size=10), zerolinecolor=PBI_BORDER),
    yaxis=dict(gridcolor="rgba(42,42,90,0.6)", linecolor=PBI_BORDER,
               tickfont=dict(color=PBI_MUTED, size=10), zerolinecolor=PBI_BORDER),
    hoverlabel=dict(bgcolor=PBI_SURFACE, bordercolor=PBI_ACCENT,
                    font=dict(color=PBI_TEXT, size=11)),
)

# ══════════════════════════════════════════════════════════════
#  GLOBAL CSS — VIBRANT PROFESSIONAL REDESIGN
# ══════════════════════════════════════════════════════════════
st.markdown(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,300..800;1,14..32,300..800&family=JetBrains+Mono:wght@400;500;600&display=swap');
...
.main {{ background: {PBI_BG}; }}
::-webkit-scrollbar {{ width: 5px; height: 5px; }}
::-webkit-scrollbar-track {{ background: {PBI_BG}; }}
::-webkit-scrollbar-thumb {{ background: {PBI_BORDER}; border-radius: 10px; }}
::-webkit-scrollbar-thumb:hover {{ background: {PBI_ACCENT}; }}
...
.dv-topbar {{
    ...
    border-bottom: 1px solid {PBI_BORDER};
...
.dv-logo {{
    ...
    padding-right: 28px; border-right: 1px solid {PBI_BORDER};
    flex-shrink: 0;
}}
.dv-logo-mark {{
    width: 34px; height: 34px; border-radius: 10px;
    background: linear-gradient(135deg, {PBI_ACCENT}, {PBI_PURPLE});
    display: flex; align-items: center; justify-content: center;
    font-size: 14px; font-weight: 800; color: white;
    box-shadow: 0 0 20px rgba(99,102,241,0.5);
}}
.dv-logo-name {{ font-size: 0.92rem; font-weight: 800; color: {PBI_WHITE}; letter-spacing: -0.02em; }}
.dv-logo-sub  {{ font-size: 0.6rem; color: {PBI_MUTED}; letter-spacing: 0.05em; text-transform: uppercase; }}
.dv-topbar-right {{ margin-left: auto; display: flex; align-items: center; gap: 12px; }}
.dv-pill {{
    font-size: 0.62rem; font-weight: 700; padding: 4px 12px; border-radius: 20px;
    letter-spacing: 0.08em; text-transform: uppercase;
}}
.dv-pill-accent {{ background: rgba(99,102,241,0.15); color: {PBI_ACCENT}; border: 1px solid rgba(99,102,241,0.3); }}
.dv-pill-time   {{ background: rgba(255,255,255,0.04); color: {PBI_MUTED}; border: 1px solid {PBI_BORDER}; }}

/* ════════════════════════════════════
   CANVAS
════════════════════════════════════ */
.dv-canvas {{ padding: 20px 24px 32px; background: {PBI_BG}; }}

/* ════════════════════════════════════
   SECTION HEADERS — vivid gradient label
════════════════════════════════════ */
.dv-section {{
...
.dv-section-label {{
    ...
    background: linear-gradient(90deg, {PBI_ACCENT}, {PBI_BLUE});
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
    background-clip: text; white-space: nowrap;
}}
.dv-section-line {{
    flex: 1; height: 1px;
    background: linear-gradient(to right, {PBI_BORDER}, transparent);
}}

/* ════════════════════════════════════
   KPI CARDS — gradient border glow
════════════════════════════════════ */
.dv-kpi-grid {{
    display: grid;
...
.dv-kpi {{
    background: {PBI_SURFACE};
    border-radius: 16px; padding: 18px 20px;
    position: relative; overflow: hidden;
    border: 1px solid {PBI_BORDER};
    transition: transform 0.22s ease, box-shadow 0.22s ease;
}}
.dv-kpi:hover {{
    transform: translateY(-5px);
    box-shadow: 0 12px 32px rgba(0,0,0,0.4), 0 0 0 1px var(--gc1, {PBI_ACCENT});
}}
.dv-kpi-glow {{
    position: absolute; top: -30px; right: -30px;
    width: 90px; height: 90px; border-radius: 50%;
    background: radial-gradient(circle, var(--gc1, {PBI_ACCENT}) 0%, transparent 70%);
    opacity: 0.12; pointer-events: none;
}}
.dv-kpi-stripe {{
    position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: linear-gradient(90deg, var(--gc1, {PBI_ACCENT}), var(--gc2, {PBI_PURPLE}));
    border-radius: 16px 16px 0 0;
}}
.dv-kpi-icon {{
    font-size: 1.4rem; margin-bottom: 10px;
    filter: drop-shadow(0 0 8px var(--gc1, {PBI_ACCENT}));
}}
.dv-kpi-label {{
    font-size: 0.65rem; font-weight: 700; color: {PBI_MUTED};
    text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 6px;
}}
.dv-kpi-value {{
    font-family: 'JetBrains Mono', monospace;
    font-size: 1.75rem; font-weight: 700; color: {PBI_WHITE};
    line-height: 1; letter-spacing: -0.04em; margin-bottom: 6px;
}}
.dv-kpi-meta {{
    font-size: 0.68rem; color: {PBI_MUTED}; display: flex; gap: 6px; align-items: center;
}}
.dv-kpi-meta .up   {{ color: {PBI_GREEN}; font-weight: 600; }}
.dv-kpi-meta .down {{ color: {PBI_RED}; font-weight: 600; }}

/* ════════════════════════════════════
   CHART TILES — glass card with top color stripe
════════════════════════════════════ */
.dv-tile {{
    background: {PBI_SURFACE};
    border: 1px solid {PBI_BORDER};
...
.dv-tile:hover {{
    ...
    border-color: var(--tc, {PBI_ACCENT});
}}
.dv-tile-header {{
...
.dv-tile-title {{
    ...
    text-transform: uppercase; letter-spacing: 0.07em; color: {PBI_TEXT};
}}
.dv-tile-chip {{
    width: 8px; height: 8px; border-radius: 50%;
    background: var(--tc, {PBI_ACCENT});
    box-shadow: 0 0 8px var(--tc, {PBI_ACCENT});
    flex-shrink: 0;
}}
.dv-tile-actions {{ color: {PBI_MUTED}; font-size: 0.75rem; opacity: 0.5; cursor: default; }}
.dv-tile-body {{ padding: 6px 4px 4px; }}

/* ════════════════════════════════════
   QUALITY BANNER
════════════════════════════════════ */
.dv-quality {{
    background: {PBI_SURFACE}; border: 1px solid {PBI_BORDER}; border-radius: 16px;
    padding: 18px 22px; display: flex; align-items: center; gap: 20px;
    margin-bottom: 20px; position: relative; overflow: hidden;
}}
.dv-quality::before {{
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px;
    background: linear-gradient(90deg, {PBI_ACCENT}, {PBI_BLUE}, {PBI_GREEN});
}}
.dv-quality-ring {{
    width: 68px; height: 68px; border-radius: 50%; flex-shrink: 0;
    border: 3px solid var(--qc, {PBI_GREEN});
    display: flex; align-items: center; justify-content: center;
    font-size: 1.1rem; font-weight: 800; color: var(--qc, {PBI_GREEN});
    box-shadow: 0 0 20px rgba(52,211,153,0.2);
    font-family: 'JetBrains Mono', monospace;
}}
.dv-quality-info h3 {{ font-size: 0.95rem; font-weight: 700; color: {PBI_WHITE}; margin-bottom: 4px; }}
.dv-quality-info p  {{ font-size: 0.74rem; color: {PBI_MUTED}; margin-bottom: 10px; }}
.dv-chips {{ display: flex; flex-wrap: wrap; gap: 6px; }}
.dv-chip {{
    background: rgba(255,255,255,0.05);
    border: 1px solid {PBI_BORDER};
    border-radius: 20px; padding: 3px 11px;
    font-size: 0.66rem; color: {PBI_TEXT}; white-space: nowrap;
}}
...
.dv-insight {{
    background: {PBI_SURFACE}; border: 1px solid {PBI_BORDER};
    border-left: 3px solid var(--ic, {PBI_ACCENT});
    border-radius: 12px; padding: 12px 14px;
    display: flex; gap: 10px; align-items: flex-start;
    font-size: 0.8rem; color: {PBI_TEXT}; line-height: 1.5;
    transition: background 0.15s, transform 0.15s;
}}
.dv-insight:hover {{ background: rgba(255,255,255,0.03); transform: translateX(3px); }}
.dv-insight .i-icon {{ font-size: 1.1rem; flex-shrink: 0; margin-top: 1px; }}
.dv-insight b {{ color: var(--ic, {PBI_ACCENT2}); }}

/* ════════════════════════════════════
   SIDEBAR
════════════════════════════════════ */
[data-testid="stSidebar"] {{
    background: {PBI_SIDEBAR} !important;
    border-right: 1px solid {PBI_BORDER} !important;
    min-width: 260px !important;
    max-width: 280px !important;
}}
[data-testid="stSidebarContent"] {{ padding: 16px !important; }}
/* ── Pin sidebar: hide collapse button, keep always open ── */
[data-testid="collapsedControl"] {{ display: none !important; }}
[data-testid="stSidebarCollapseButton"] {{ display: none !important; }}
section[data-testid="stSidebar"] > div:first-child {{
    width: 270px !important;
}}
/* Prevent sidebar from ever being in collapsed state */
...
.dv-sidebar-head h2 {{ font-size: 0.82rem; font-weight: 800; color: {PBI_WHITE}; margin-bottom: 2px; letter-spacing: -0.01em; }}
.dv-sidebar-head span {{ font-size: 0.65rem; color: {PBI_MUTED}; }}
...
.stNumberInput label, .stDateInput label {{
    color: {PBI_TEXT} !important; font-size: 0.72rem !important;
    font-weight: 700 !important; text-transform: uppercase;
    letter-spacing: 0.06em !important;
}}
div[data-baseweb="select"] > div {{
    background: rgba(255,255,255,0.04) !important;
    border: 1px solid {PBI_BORDER} !important;
    border-radius: 10px !important; color: {PBI_TEXT} !important;
    font-size: 0.83rem !important;
}}
div[data-baseweb="select"] > div:focus-within {{
    border-color: {PBI_ACCENT} !important;
...
/* ════════════════════════════════════
   TABS  — vivid underline style
════════════════════════════════════ */
.stTabs [data-baseweb="tab-list"] {{
    background: transparent !important;
    border-bottom: 1px solid {PBI_BORDER} !important;
    gap: 0 !important; padding: 0 !important;
}}
.stTabs [data-baseweb="tab"] {{
    color: {PBI_MUTED} !important; font-weight: 700 !important;
    font-size: 0.72rem !important; text-transform: uppercase !important;
    letter-spacing: 0.06em !important; padding: 12px 20px !important;
    border-radius: 0 !important; border-bottom: 2px solid transparent !important;
    background: transparent !important; transition: all 0.15s !important;
}}
.stTabs [data-baseweb="tab"]:hover {{
    color: {PBI_TEXT} !important;
    background: rgba(255,255,255,0.03) !important;
}}
.stTabs [aria-selected="true"] {{
    color: {PBI_WHITE} !important;
    border-bottom-color: {PBI_ACCENT} !important;
...
════════════════════════════════════ */
.stButton > button {{
    background: linear-gradient(135deg, {PBI_ACCENT}, {PBI_PURPLE}) !important;
...
.stDownloadButton > button {{
    background: transparent !important; color: {PBI_ACCENT} !important;
    border: 1.5px solid {PBI_ACCENT} !important; border-radius: 10px !important;
...
[data-testid="stMetric"] {{
    background: {PBI_SURFACE} !important; padding: 14px 16px !important;
    border-radius: 12px !important; border: 1px solid {PBI_BORDER} !important;
    border-top: 2px solid {PBI_ACCENT} !important;
}}
[data-testid="stMetricLabel"] {{
    color: {PBI_MUTED} !important; font-size: 0.65rem !important;
    text-transform: uppercase !important; letter-spacing: 0.08em !important; font-weight: 700 !important;
}}
[data-testid="stMetricValue"] {{
    color: {PBI_WHITE} !important; font-size: 1.5rem !important;
...
/* ════════════════════════════════════
   PROGRESS / ALERTS / EXPANDERS
════════════════════════════════════ */
.stProgress > div > div > div {{
    background: linear-gradient(90deg, {PBI_ACCENT}, {PBI_BLUE}) !important;
    border-radius: 4px !important;
}}
.stAlert {{ border-radius: 12px !important; font-size: 0.8rem !important; border: none !important; }}
.streamlit-expanderHeader {{
    background: {PBI_SURFACE} !important; border-radius: 10px !important;
    color: {PBI_TEXT} !important; font-size: 0.79rem !important; font-weight: 700 !important;
}}
.stDataFrame {{ border-radius: 12px !important; overflow: hidden !important; }}
[data-testid="stCheckbox"] span {{ color: {PBI_TEXT} !important; font-size: 0.82rem !important; }}

/* ════════════════════════════════════
   CHAT
════════════════════════════════════ */
.dv-chat {{ display: flex; flex-direction: column; gap: 12px; margin-bottom: 16px; }}
.dv-chat-row {{ display: flex; align-items: flex-end; gap: 8px; }}
.dv-chat-row.user {{ flex-direction: row-reverse; }}
.dv-bubble {{
...
.dv-bubble.ai {{
    background: {PBI_SURFACE}; color: {PBI_TEXT};
    border: 1px solid {PBI_BORDER}; border-radius: 14px 14px 14px 4px;
}}
.dv-avatar {{
...
.dv-avatar.user {{ background: linear-gradient(135deg, {PBI_ACCENT}, {PBI_PURPLE}); }}
.dv-avatar.ai   {{ background: {PBI_SURFACE}; border: 1px solid {PBI_BORDER}; }}
.dv-chat-empty {{
    text-align: center; padding: 40px 24px;
    background: {PBI_SURFACE}; border: 1px solid {PBI_BORDER}; border-radius: 16px;
    color: {PBI_MUTED};
}}
.dv-chat-empty .ce-icon {{ font-size: 2.4rem; margin-bottom: 10px; }}
.dv-chat-empty h3 {{ color: {PBI_WHITE}; font-size: 1rem; margin-bottom: 6px; }}
.dv-chat-empty p {{ font-size: 0.8rem; line-height: 1.5; }}
...
.dv-welcome-card::before {{
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px;
    background: linear-gradient(90deg, var(--wc1, {PBI_ACCENT}), var(--wc2, {PBI_PURPLE}));
}}
.dv-welcome-card:hover {{
    transform: translateY(-6px);
    border-color: var(--wc1, {PBI_ACCENT});
    box-shadow: 0 12px 36px rgba(0,0,0,0.4);
}}
.dv-welcome-icon {{ font-size: 2.2rem; margin-bottom: 12px; }}
.dv-welcome-title {{ color: {PBI_WHITE}; font-weight: 800; font-size: 0.9rem; margin-bottom: 6px; }}
.dv-welcome-desc  {{ color: {PBI_MUTED}; font-size: 0.74rem; line-height: 1.5; }}
...
.dv-footer {{
    ...
    color: rgba(100,116,139,0.5); font-size: 0.66rem;
    border-top: 1px solid {PBI_BORDER}; margin-top: 28px;
...
def apply_theme(fig, height=None):
    ...
    layout["transition"] = {"duration": 0}        # no animations = faster render
    layout["uirevision"] = "constant"             # prevent full re-render on filter
...
# ══════════════════════════════════════════════════════════════
_CHUNK        = 200_000   # pandas chunk rows (fallback)
_DASK_THRESH  = 50        # MB — use Dask above this
_PARQUET_DIR  = os.path.join(tempfile.gettempdir(), "dvpro_cache")
os.makedirs(_PARQUET_DIR, exist_ok=True)

# ── Vectorised dtype downcast — groups columns, avoids per-row overhead ──
def _downcast(df: pd.DataFrame) -> pd.DataFrame:
    """Shrink numeric dtypes — cuts RAM 40-60%. Vectorised per dtype group."""
...
# ── Parquet cache: hash file bytes → skip re-parse on same session ────────
def _parquet_cache_path(file_bytes: bytes, file_name: str) -> str:
    h = __import__("hashlib").md5(file_bytes[:65536] + file_name.encode()).hexdigest()
    return os.path.join(_PARQUET_DIR, f"{h}.parquet")

# ── Dask-powered CSV reader (out-of-core, handles 1 GB+) ─────────────────
def _csv_dask(raw_bytes: bytes, encoding: str = "utf-8") -> pd.DataFrame:
    """Write bytes to temp file, read with Dask in parallel chunks, compute."""
    import dask.dataframe as dd
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".csv")
    try:
        tmp.write(raw_bytes); tmp.flush(); tmp.close()
        ddf = dd.read_csv(
            tmp.name, encoding=encoding, low_memory=False,
            on_bad_lines="warn", blocksize="64MB",
            assume_missing=True,
        )
        df = ddf.compute()
    finally:
...
def _csv_chunked(buf, encoding: str = "utf-8") -> pd.DataFrame:
    """Stream CSV in _CHUNK-row batches. dtype inference only on first chunk."""
    chunks = []; dtypes = None
    reader = pd.read_csv(buf, encoding=encoding, low_memory=False,
                         chunksize=_CHUNK, on_bad_lines="warn")
    for chunk in reader:
        if dtypes is None:
            ...
            for col in chunk.columns:
                if col in dtypes.index:
                    try: chunk[col] = chunk[col].astype(dtypes[col])
                    except: pass
        chunks.append(chunk)
    if not chunks:
...
# ── Master reader — Dask → chunked pandas → Parquet cache ─────────────────
def safe_read(file_bytes: bytes, file_name: str) -> dict:
    """
    Priority chain:
      1. Parquet cache hit  →  instant re-load from disk
      2. CSV > _DASK_THRESH →  Dask parallel read (handles 1 GB+)
      3. CSV ≤ _DASK_THRESH →  chunked pandas (low-memory)
      4. Excel              →  calamine (Rust, 5× faster) or openpyxl
    All paths apply _downcast + date detection, then cache to Parquet.
    """
    name   = file_name.lower()
    mb     = len(file_bytes) / 1_048_576
    cache  = _parquet_cache_path(file_bytes, file_name)

    # ── Fast path: cached parquet ──────────────────────────────────────────
    if os.path.exists(cache):
        try:
            df = pd.read_parquet(cache)
            return {"Sheet1": df} if name.endswith(".csv") else _parquet_multi_sheet(cache)
        except Exception:
            pass  # stale / corrupt cache → re-parse
    ...
    if name.endswith(".csv"):
        ...
        for enc in ["utf-8", "utf-8-sig", "latin-1", "cp1252"]:
            try:
                if mb > _DASK_THRESH:
                    df = _csv_dask(file_bytes, encoding=enc)
                    df = detect_and_parse_dates(df)
                    df = _downcast(df)
                else:
                    buf = io.BytesIO(file_bytes); buf.seek(0)
                    df = _csv_chunked(buf, encoding=enc)
                # Write Parquet cache (schema-safe: convert categories to strings first)
                try:
                    df_cache = df.copy()
                    for c in df_cache.select_dtypes(include="category").columns:
                        df_cache[c] = df_cache[c].astype(str)
                    df_cache.to_parquet(cache, index=False, compression="snappy")
                    del df_cache
                except Exception:
                    pass
                return {"Sheet1": df}
                ...
                last_err = e; continue
        raise ValueError(f"CSV read error: {last_err}")

    else:  # Excel
        buf = io.BytesIO(file_bytes); buf.seek(0)
        try:
            import python_calamine; engine = "calamine"
        except ImportError:
            engine = "openpyxl"
        ...
        for s in xls.sheet_names:
            try:
                ...
                df_s = detect_and_parse_dates(df_s)
                df_s = _downcast(df_s)
                sheets[s] = df_s
            except Exception:
                pass
        ...
        # Cache first sheet only
        try:
            first = list(sheets.values())[0].copy()
            for c in first.select_dtypes(include="category").columns:
                first[c] = first[c].astype(str)
            first.to_parquet(cache, index=False, compression="snappy")
            del first
        except Exception:
            pass
        return sheets

def _parquet_multi_sheet(cache_path: str) -> dict:
    """Load cached parquet as a single-sheet dict."""
    return {"Sheet1": pd.read_parquet(cache_path)}
...
def scatter(df, x, y, color=None, size=None, title="", trendline=False):
    # Cap at 5000 pts — GPU WebGL renderer handles large data
    s = df.sample(min(5_000, len(df)), random_state=42) if len(df) > 5_000 else df
    safe_size = None
    if size and size in s.columns:
        if pd.api.types.is_numeric_dtype(s[size]) and s[size].dropna().min() >= 0:
            safe_size = size
    use_trendline = False
    if trendline:
        try:
    ...
    try:
        fig = px.scatter(s, x=x, y=y, color=color, size=safe_size, title=title,
                         color_discrete_sequence=PALETTE, opacity=0.65,
                         render_mode="webgl", **kw)
        if use_trendline:
            fig.update_traces(selector=dict(mode="lines"),
                              line=dict(color=GOLD, width=2, dash="dot"))
    except Exception:
        fig = px.scatter(s, x=x, y=y, color=color, title=title,
                         color_discrete_sequence=PALETTE, opacity=0.65)
    return apply_theme(fig)

def histogram(df, col, bins=30, color_col=None, title=""):
    s = df.sample(min(50_000, len(df)), random_state=42) if len(df) > 50_000 else df
    fig = px.histogram(s, x=col, nbins=bins, color=color_col,
                       title=title, color_discrete_sequence=PALETTE,
                       marginal="box")
...
# ══════════════════════════════════════════════════════════════
# sec() replaced by pbi_section() — kept for compatibility
def sec(icon, title, count=None):
    pbi_section(f"{icon} {title}")



# ══════════════════════════════════════════════════════════════
#  POWER BI CARD / SECTION HELPERS
# ══════════════════════════════════════════════════════════════
def pbi_section(label):
    st.markdown(f"""
    <div class="dv-section">
        <div class="dv-section-label">{label}</div>
        <div class="dv-section-line"></div>
    </div>""", unsafe_allow_html=True)

def pbi_card(title, dot_color=None, color2=None):
    if dot_color is None: dot_color = PBI_ACCENT
    if color2    is None: color2    = PBI_PURPLE
    st.markdown(f"""
    <div class="dv-tile" style="--tc:{dot_color}">
        <div class="dv-tile-header">
            <div class="dv-tile-title">
                <div class="dv-tile-chip"></div>
                {title}
            </div>
            <span class="dv-tile-actions">&#8943;</span>
        </div>
        <div class="dv-tile-body">
    """, unsafe_allow_html=True)
...
<div class="dv-topbar">
    <div class="dv-logo">
        <div class="dv-logo-mark">DV</div>
        <div>
            <div class="dv-logo-name">DataVision Pro</div>
            <div class="dv-logo-sub">Enterprise Analytics</div>
        </div>
    </div>
    <div class="dv-topbar-right">
        <span class="dv-pill dv-pill-accent">v4.0 Expert</span>
        <span class="dv-pill dv-pill-time">&#x1F550; {_now_str}</span>
    </div>
</div>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
#  SIDEBAR — upload + plan + filters (single block)
# ══════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown(f"""
    <div class="dv-sidebar-head">
        <h2>&#9646; DataVision Pro</h2>
        <span>Filter Panel &amp; Controls</span>
    </div>""", unsafe_allow_html=True)

    # Plan
    st.markdown("**💎 Plan**")
    plan = st.radio("Plan", ["🆓 Free", "⭐ Pro", "🏢 Business"],
                    index=1, label_visibility="collapsed", key="plan_sel")
    PLAN      = plan.split()[-1]
    ROW_LIMIT = {"Free": 300, "Pro": 10_000, "Business": 9_999_999}[PLAN]
    st.caption(f"**{PLAN}** plan · Row limit: {ROW_LIMIT:,}")
    st.markdown("---")
    ...
    use_sample = False
    if uploaded is None:
        st.caption("No file yet — or load the demo:")
    ...
    try:
        mem_mb = df_raw.memory_usage(deep=True).sum() / 1_048_576
        cache_hit = os.path.exists(_parquet_cache_path(file_bytes if not use_sample else b"", uploaded.name if not use_sample else ""))
        engine_used = "Dask" if (not use_sample and len(file_bytes)/1_048_576 > _DASK_THRESH) else "Pandas"
        st.markdown(f"""
<div style="background:rgba(99,102,241,0.08);border:1px solid {PBI_BORDER};
            border-radius:8px;padding:10px 12px;font-size:0.68rem;color:{PBI_MUTED}">
    <div style="color:{PBI_ACCENT};font-weight:700;margin-bottom:4px">⚡ Performance</div>
    <div>Engine: <b style="color:{PBI_TEXT}">{engine_used}</b></div>
    <div>Rows in memory: <b style="color:{PBI_TEXT}">{len(df_raw):,}</b></div>
    <div>Chart sample: <b style="color:{PBI_TEXT}">{min(len(df_chart),200_000):,}</b></div>
    <div>RAM usage: <b style="color:{PBI_TEXT}">{mem_mb:.1f} MB</b></div>
    <div>Parquet cache: <b style="color:{'#34d399' if cache_hit else PBI_MUTED}">{'✓ HIT' if cache_hit else '○ MISS'}</b></div>
</div>""", unsafe_allow_html=True)
    except Exception:
        pass
    st.caption("DataVision Pro v4.0 · Dask + Parquet Engine")
...
if uploaded is None and not use_sample:
    st.markdown(f"""
    <div class="dv-canvas">
    <div style="text-align:center;padding:48px 24px 20px">
        <div style="font-size:3rem;margin-bottom:14px">&#x26AB;</div>
        <div style="font-size:1.4rem;font-weight:800;color:{PBI_WHITE};margin-bottom:8px">
            DataVision Pro
        </div>
        <div style="font-size:0.85rem;color:{PBI_MUTED};max-width:500px;margin:0 auto;line-height:1.6">
            Upload a <b style="color:{PBI_TEXT}">CSV or Excel file</b> using the sidebar on the left,
            or click <b style="color:{PBI_ACCENT}">Load Demo Dataset</b> to explore with sample data.
        </div>
    </div>
    <div class="dv-welcome-grid">
        <div class="dv-welcome-card" style="--wc1:{PBI_ACCENT};--wc2:{PBI_PURPLE}">
            <div class="dv-welcome-icon">📂</div>
            <div class="dv-welcome-title">Upload Any File</div>
            <div class="dv-welcome-desc">CSV or Excel — up to 1 GB+ via Dask out-of-core engine</div>
        </div>
        <div class="dv-welcome-card" style="--wc1:{PBI_BLUE};--wc2:{PBI_TEAL}">
            <div class="dv-welcome-icon">📊</div>
            <div class="dv-welcome-title">15+ Chart Types</div>
            <div class="dv-welcome-desc">Bar, Line, Scatter, Treemap, Donut, Pareto &amp; more</div>
        </div>
        <div class="dv-welcome-card" style="--wc1:{PBI_GREEN};--wc2:{PBI_LIME}">
            <div class="dv-welcome-icon">🔮</div>
            <div class="dv-welcome-title">AI Forecasting</div>
            <div class="dv-welcome-desc">Time-series forecasting with confidence bands</div>
        </div>
        <div class="dv-welcome-card" style="--wc1:{PBI_PURPLE};--wc2:{PBI_PINK}">
            <div class="dv-welcome-icon">🤖</div>
            <div class="dv-welcome-title">AI Chat Analyst</div>
            <div class="dv-welcome-desc">Ask questions in English or Arabic</div>
        </div>
        <div class="dv-welcome-card" style="--wc1:{PBI_TEAL};--wc2:{PBI_BLUE}">
            <div class="dv-welcome-icon">🧹</div>
            <div class="dv-welcome-title">Data Cleaning</div>
            <div class="dv-welcome-desc">Remove duplicates, fill missing values, rename columns</div>
        </div>
        <div class="dv-welcome-card" style="--wc1:{PBI_ORANGE};--wc2:{PBI_ACCENT2}">
            <div class="dv-welcome-icon">📤</div>
            <div class="dv-welcome-title">Export Center</div>
            <div class="dv-welcome-desc">CSV, Excel, HTML report &amp; interactive charts</div>
        </div>
    </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
#  LOAD DATA
# ══════════════════════════════════════════════════════════════
@st.cache_data(show_spinner="⚡ Parsing data…", ttl=7200, max_entries=2)
def load_data(file_bytes: bytes, file_name: str):
    """
    st.cache_data layer — keyed on (file_bytes hash, file_name).
    safe_read handles Dask / chunked-pandas / Excel internally.
    Parquet cache inside safe_read gives instant re-loads within the same
    OS session even after Streamlit reruns.
    """
    return safe_read(file_bytes, file_name)
...
if use_sample:
    ...
    st.info("📊 **Demo mode** — 500-row sales dataset. Upload your own CSV/Excel via the sidebar.")
else:
    try:
        ...
        if file_mb > 500:
            st.warning(
                f"⚠️ **Very large file ({file_mb:.0f} MB).** "
                "Streamlit's default upload cap is 200 MB. To raise it, run: "
                "`streamlit run app.py --server.maxUploadSize=2048`"
            )
        elif file_mb > _DASK_THRESH:
            st.info(
                f"📦 **Large file ({file_mb:.1f} MB)** — "
                f"{'Dask out-of-core engine' if file_mb > _DASK_THRESH else 'chunked streaming'} active. "
                "First load may take 15-40 s; subsequent loads use the Parquet cache (instant)."
            )
        with st.spinner(f"⚡ Loading {uploaded.name} ({file_mb:.1f} MB) — please wait…"):
            sheets = load_data(file_bytes, uploaded.name)
        total_rows = sum(len(v) for v in sheets.values())
        st.success(
            f"✅ **{uploaded.name}** loaded — "
            f"**{total_rows:,} rows** · {len(sheets)} sheet(s) · {file_mb:.1f} MB · RAM-optimised"
        )
    except Exception as e:
        st.error(f"❌ **File read error:** {e}")
        st.info("Tips: save CSV as UTF-8, ensure Excel has at least one data sheet, check file isn't corrupt.")
        st.stop()
...
if len(df_raw) > ROW_LIMIT:
    st.warning(f"⚠️ **{PLAN} plan:** {len(df_raw):,} rows → showing first {ROW_LIMIT:,}.")
...
# ── Sidebar filters (shown only after data is loaded) ──────────────────────
df = df_raw.copy()
with st.sidebar:
    st.markdown("---")
    st.markdown("**🔍 Filters**")
    ...
    for nc in num_cols[:2]:
        try:
            ...
            pass
    if active_filters:
        st.success(f"✅ {active_filters} filter(s) · {len(df):,} rows")
    else:
...
# ── Chart-safe sample ────────────────────────────────────────────────────
# Charts work on df_chart (≤300K rows) for speed.
# KPI totals / quality stats still use full df.
_CHART_CAP = 300_000
if len(df) > _CHART_CAP:
    st.info(
        f"⚡ **Large dataset ({len(df):,} rows):** "
        f"charts render on a **{_CHART_CAP:,}-row** stratified sample for speed. "
        "KPIs & quality metrics reflect the **full** dataset."
    )
    df_chart = df.sample(_CHART_CAP, random_state=42)
else:
    df_chart = df

# ── Cached groupby helper — LRU cache with proper df fingerprint ──────────
@st.cache_data(show_spinner=False, ttl=600, max_entries=120)
def _cgb(df_key: int, df_len: int, group_cols: tuple, val_col: str, agg: str = "sum"):
    """
    Cached groupby.
    df_key  = id(df_chart) changes when the filtered frame changes.
    df_len  = len(df_chart) extra collision guard.
    """
    return safe_groupby(df_chart, list(group_cols), val_col, agg)

def cgb(group_cols, val_col, agg="sum"):
    """Convenience wrapper — auto-passes cache key."""
    cols = (group_cols,) if isinstance(group_cols, str) else tuple(group_cols)
    return _cgb(id(df_chart), len(df_chart), cols, val_col, agg)
...
with tab_overview:
    ...
    qc  = PBI_GREEN if qs >= 80 else (PBI_ACCENT2 if qs >= 50 else PBI_ACCENT)
    qcg = hex_to_rgba(qc, 0.2)
    ql  = "Excellent" if qs >= 80 else ("Needs Attention" if qs >= 50 else "Poor Quality")
    st.markdown(f"""
    <div class="dv-quality">
        <div class="dv-quality-ring" style="--qc:{qc}">{qs:.0f}</div>
        <div class="dv-quality-info">
            <h3>{"✅" if qs>=80 else "⚠️"} Data Quality: {ql}</h3>
            <p>{len(df_raw):,} rows · {len(df_raw.columns)} columns analyzed</p>
            <div class="dv-chips">
                <span class="dv-chip">📋 {len(df_raw):,} rows</span>
                <span class="dv-chip">📑 {len(df_raw.columns)} cols</span>
                <span class="dv-chip">❓ {df_raw.isnull().mean().mean()*100:.1f}% missing</span>
                <span class="dv-chip">🔁 {df_raw.duplicated().sum()} dups</span>
                <span class="dv-chip">📊 {len(num_cols)} numeric</span>
                <span class="dv-chip">🏷️ {len(cat_cols)} text</span>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if num_cols:
        ...
        accents = PALETTE
        icons   = ["💰","📦","📊","🎯","⚡","🔥","💡","📐"]
        KC_LIST = [PBI_ACCENT, PBI_BLUE, PBI_GREEN, PBI_ACCENT2, PBI_PURPLE, PBI_TEAL, PBI_ORANGE, PBI_ACCENT]
        cards_html = '<div class="dv-kpi-grid">'
        for i, col in enumerate(num_cols[:8]):
            ...
            kc2 = KC_LIST[(i+2) % len(KC_LIST)]
            cards_html += f"""
            <div class="dv-kpi" style="--gc1:{kc};--gc2:{kc2}">
                <div class="dv-kpi-glow"></div>
                <div class="dv-kpi-stripe"></div>
                <div class="dv-kpi-icon">{icons[i%len(icons)]}</div>
                <div class="dv-kpi-label">{col}</div>
                <div class="dv-kpi-value">{fmt(total)}</div>
                <div class="dv-kpi-meta">Avg {fmt(mean_)} &nbsp;·&nbsp; Max {fmt(mx_v)}</div>
            </div>"""
        cards_html += "</div>"
    ...
    if cat_cols and num_cols:
        ...
        with c1:
            try:
                d = safe_groupby(df_chart, [cat_cols[0]], num_cols[0])
    ...
    if date_cols and num_cols:
        try:
            d   = safe_groupby(df_chart, [date_cols[0]], num_cols[0]).sort_values(date_cols[0])
    ...
    # Auto insights
    sec("🤖", "Auto Insights")
    insights = auto_insights(df_chart, num_cols, cat_cols, date_cols)
    ins_html = '<div class="dv-insights">'
    for icon, color, text in insights:
        clean = text.replace("**","<b>",1)
        while "**" in clean:
            clean = clean.replace("**","</b>",1).replace("**","<b>",1)
        ins_html += f'<div class="dv-insight" style="--ic:{color}"><div class="i-icon">{icon}</div><div>{clean}</div></div>'
    ins_html += "</div>"
...
with tab_charts:
    ...
    try:
        if chart_type in ["Horizontal Bar","Vertical Bar","Grouped Bar",
            ...
            if chart_type in ["Horizontal Bar","Vertical Bar","Grouped Bar"]:
                d   = safe_groupby(df_chart, [x_col]+([color_col] if color_col else []), y_col)
                ...
                fig = waterfall(df_chart, x_col, y_col, f"{y_col} Waterfall")
            elif chart_type == "Funnel":
                fig = funnel(df, x_col, y_col, f"{y_col} Funnel")
            elif chart_type == "Pareto Analysis":
                fig = pareto_chart(df_chart, x_col, y_col)
            st.plotly_chart(fig, use_container_width=True)
            ...
            d    = safe_groupby(df_chart, [names_c], vals_c)
            ...
            col_c = None if col_c == "None" else col_c
            fig   = histogram(df_chart, col_h, bins_h, col_c, f"{col_h} Distribution")
            st.plotly_chart(fig, use_container_width=True)
            chart_download_btn(fig)
            ...
            fig = box_plot(df_chart, y_c, x_c, f"{y_c} Distribution")
            st.plotly_chart(fig, use_container_width=True)
            chart_download_btn(fig)
...
with tab_analysis:
    ...
    if len(num_cols) >= 2:
        fig_corr = heatmap_corr(df_chart, num_cols)
        if fig_corr:
    ...
    with c1:
        ...
        if cat_cols and num_cols:
            ...
            if path_sel:
                try:
                    st.plotly_chart(treemap(df_chart, path_sel, val_sel), use_container_width=True)
                except Exception as e:
    ...
    with c2:
        ...
        if len(cat_cols) >= 2 and num_cols:
            ...
            if r_col != c_col:
                try:
                    st.plotly_chart(pivot_heatmap(df_chart, r_col, c_col, v_col), use_container_width=True)
                except Exception as e:
    ...
    if date_cols and num_cols:
        ...
        if met_sel:
            try:
                st.plotly_chart(multi_axis_line(df_chart, dc_sel, met_sel), use_container_width=True)
            except Exception as e:
                st.warning(f"Time series error: {e}")
    ...
    if num_cols:
        try:
            stats = df_chart[num_cols].describe().T.round(2)
            stats["skewness"] = df_chart[num_cols].skew().round(2)
            stats["kurtosis"] = df_chart[num_cols].kurt().round(2)
...
with tab_deep:
    ...
    if num_cols:
        ...
        try:
            ...
            with c1:
                st.plotly_chart(outlier_scatter(df_chart, out_col), use_container_width=True)
            with c2:
                s     = df[out_col].dropna()
                mask  = detect_outliers_iqr(s)
                n_out = mask.sum()
    ...
    if cat_cols and num_cols:
        ...
        try:
            st.plotly_chart(pareto_chart(df_chart, p_cat, p_val, f"Pareto: {p_val} by {p_cat}"),
                            use_container_width=True)
        except Exception as e:
...
with tab_export:
    ...
    with e3:
        ...
        if num_cols:
            stats_exp = df_chart[num_cols].describe().T.copy()
            stats_exp["skew"]     = df_chart[num_cols].skew()
            stats_exp["kurtosis"] = df_chart[num_cols].kurt()
...
st.markdown(f"""
<div class="dv-footer">
    DataVision Pro · Expert Edition v4.0 · Streamlit · Plotly · Claude AI · Dask Engine &nbsp;·&nbsp; {datetime.now().year}
</div>""", unsafe_allow_html=True)
